...
We are interested in the modeling results around the modeling period which starts at year 1952. To get a meaniningful "interpretable" intercept we can use the `I()` function. Which creates new variable on the fly. 
We've created a variable on the fly to shift the y axis to 1952
```{r}
...
lm, use just for creating ggplot, even though same method as before.
se argument shows standard errors. 
...
This relationship better not to be modelled by linear model...
```{r}
...
OR we cn make some modifications and try fitting a linear model to this relationship
```{r}
