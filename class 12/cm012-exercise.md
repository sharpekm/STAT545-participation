...
gapminder %>% 
  filter(year == 1997) %>% 
  mutate(life_level = factor(case_when(lifeExp < 23 ~ "very low", #make it a factor
                                lifeExp < 48 ~ "low",
                                lifeExp < 59 ~ "moderate",
                                lifeExp < 70 ~ "high",
                                TRUE ~ "very high"),
                             levels = c("very low", "low", "moderate", "high", "very high"))) %>% #must specify the levels of the factor
  ggplot() + geom_boxplot(aes(x = life_level, y = gdpPercap)) +
  labs(y = "GDP per capita, $", x= "Life expectancy level, years") +
...
Sees we have factor called "very low" but because none of them fall into that level, there is nothing to plot (in the previous plot it dropped the level completely)
...
```r
class(gapminder$continent)  ## continent is stored as a factor
```
...
```
## Error in eval(lhs, parent, parent): object 'gap' not found
```
...
```
## Error in eval(lhs, parent, parent): object 'h_gap_dropped' not found
```
...
gap_drop_2 <- gap2 %>% mutate(country=forcats::fct_drop(gm$country))
```

```
## Error in check_factor(f): object 'gm' not found
```

```r
gap_drop_1$country %>% nlevels()
```
...
```r
gap_drop_1$continent %>% nlevels() #in the first way the extra factors were dropped from continent as well
```
...
```r
gap_drop_2$country %>% nlevels()
```

```
## Error in eval(lhs, parent, parent): object 'gap_drop_2' not found
```

```r
gap_drop_2$continent %>% nlevels() #the second way acts on country only and keeps all continent levels
```
