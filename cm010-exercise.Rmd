...
Want filtering type join because care about the data in 'time' 
...
Want a mutating join because want to see the album this time...
...
Other joins so far have gotten rid of data where it doesn't match up... but we want to keep it and get na
...
```{r}
time %>% 
  full_join(album, by = c("song", "artist name"))
```
...
anti_join(y, z)
```
setdiff works here because of same number of columns 